...
AllGPSSources = [
    {
        ...
        "GPSSources": [
            {
                "device":"DPS122",
                "regex":"_dps122_gga-*.json"
            },
            {
                "device":"POSMV",
                "regex":"_posmv_gga-*.json"
            }
        ]
    }
]
...
if __name__ == "__main__":
    ...
    parser.add_argument('-k','--kml_only', action='store_true', help='only create the KML files')
    parser.add_argument('-g','--geojson_only', action='store_true', help='only create the GeoJSON files')
    ...
    if parsed_args.kml_only and parsed_args.geojson_only:
        parser.error("Can not specify the '--kml_only' AND '--geojson_only' arguments.")
        sys.exit(1)
