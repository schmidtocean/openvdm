...
USAGE:  mwd_parser.py [-h] [-v+] [--timeFormat] [--startDT] [--stopDT] <dataFile>

DESCRIPTION:  Parse the supplied NMEA MWD data and return the json-formatted
...
RAW_COLS = ['date_time','hdr','wind_dir_t','T','wind_dir_m','M','wind_spd_n','N','wind_spd_m','M_checksum'] # OpenRVDAS style
# RAW_COLS = ['date','time','hdr','wind_dir_t','T','wind_dir_m','M','wind_spd_n','N','wind_spd_m','M_checksum'] # SCS style
PROC_COLS = ['date_time','wind_spd_m','wind_dir_t']
...
class MWDParser(OpenVDMCSVParser):
    ...
    def process_file(self, filepath): # pylint: disable=too-many-locals,too-many-branches,too-many-statements
            ...
            with open(filepath, mode='r', encoding="utf-8") as csvfile:
                ...
                for lineno, line in enumerate(reader):
                    ...
                    try:
                        ...
                        wind_spd_m = float(line['wind_spd_m'])
                        wind_dir_t = float(line['wind_dir_t'])
                        ...
                        raw_into_df['wind_spd_m'].append(wind_spd_m)
                        raw_into_df['wind_dir_t'].append(wind_dir_t)
        ...
        self.add_bounds_stat([round(df_proc['wind_spd_m'].min(),2), round(df_proc['wind_spd_m'].max(),2)], 'Wind Speed Bounds', 'm/s')
        ...
        visualizer_data_obj['data'] = json.loads(df_proc[['date_time','wind_spd_m']].to_json(orient='values'))
        visualizer_data_obj['unit'] = 'm/s'
        visualizer_data_obj['label'] = 'Wind Speed'
        self.add_visualization_data(deepcopy(visualizer_data_obj))

        visualizer_data_obj['data'] = json.loads(df_proc[['date_time','wind_dir_t']].to_json(orient='values'))
        visualizer_data_obj['unit'] = 'deg'
        visualizer_data_obj['label'] = 'Wind Direction'
        self.add_visualization_data(deepcopy(visualizer_data_obj))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Parse true wind sensor data')
    ...
    ovdm_parser = MWDParser(start_dt=parsed_args.startDT, stop_dt=parsed_args.stopDT, time_format=parsed_args.timeFormat)
