...
USAGE:  par2_parser.py [-h] [-v+] [--timeFormat] [--startDT] [--stopDT] <dataFile>

DESCRIPTION:  Parse the supplied PAR sensor data and return the json-formatted string
...
PROC_COLS = ['date_time','lw_irradiance','sw_irradiance']
...
class PARParser(OpenVDMCSVParser):
    """
    ...
    def process_file(self, filepath): # pylint: disable=too-many-locals,too-many-branches,too-many-statements
            ...
            with open(filepath, mode='r', encoding="utf-8") as csvfile:
                ...
                for lineno, line in enumerate(reader):
                    ...
                    try:
                        ...
                        lw_irradiance = float(line['lw_irradiance'])
                        sw_irradiance = float(line['sw_irradiance'])
                        ...
                        raw_into_df['lw_irradiance'].append(lw_irradiance)
                        raw_into_df['sw_irradiance'].append(sw_irradiance)
        ...
        self.add_bounds_stat([round(df_proc['lw_irradiance'].min(),3), round(df_proc['lw_irradiance'].max(),3)], 'LW Irradiance', '')
        self.add_bounds_stat([round(df_proc['sw_irradiance'].min(),3), round(df_proc['lw_irradiance'].max(),3)], 'SW Irradiance', '')
        ...
        visualizer_data_obj['data'] = json.loads(df_proc[['date_time','lw_irradiance']].to_json(orient='values'))
        visualizer_data_obj['unit'] = '\u03BCE/cm2sec'
        visualizer_data_obj['label'] = 'LW Irradiance'
        self.add_visualization_data(deepcopy(visualizer_data_obj))

        visualizer_data_obj['data'] = json.loads(df_proc[['date_time','sw_irradiance']].to_json(orient='values'))
        visualizer_data_obj['unit'] = '\u03BCE/cm2sec'
        visualizer_data_obj['label'] = 'SW Irradiance'
        self.add_visualization_data(deepcopy(visualizer_data_obj))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Parse PAR sensor data')
    ...
    ovdm_parser = PARParser(start_dt=parsed_args.startDT, stop_dt=parsed_args.stopDT, time_format=parsed_args.timeFormat, use_openvdm_api=False)
