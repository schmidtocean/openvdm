...
USAGE:  tsg_parser.py [-h] [-v+] [--timeFormat] [--startDT] [--stopDT] <dataFile>

DESCRIPTION:  Parse the supplied SBE45 TSG w/ external SBE38 temperature data and
    return the json-formatted string used by OpenVDM as part of it's Data
    dashboard.
...
RAW_COLS = ['date_time','water_temp_int','conductivity','salinity','sound_velocity','water_temp_ext'] # OpenRVDAS style
# RAW_COLS = ['date','time','water_temp_int','conductivity','salinity','sound_velocity','water_temp_ext'] # SCS style
PROC_COLS = ['date_time','water_temp_int','conductivity','salinity','sound_velocity','water_temp_ext']
...
class TSGParser(OpenVDMCSVParser):
    """
    ...
    def process_file(self, filepath): # pylint: disable=too-many-locals,too-many-branches,too-many-statements
            ...
            with open(filepath, mode='r', encoding="utf-8") as csvfile:
                ...
                for lineno, line in enumerate(reader):
                    ...
                    try:
                        ...
                        water_temp_int = float(line['water_temp_int'].split('=')[1])
                        conductivity = float(line['conductivity'].split('=')[1])
                        salinity = float(line['salinity'].split('=')[1])
                        sound_velocity = float(line['sound_velocity'].split('=')[1])
                        water_temp_ext = float(line['water_temp_ext'].split('=')[1])
                        ...
                        raw_into_df['water_temp_int'].append(water_temp_int)
                        ...
                        raw_into_df['water_temp_ext'].append(water_temp_ext)
        ...
        self.add_bounds_stat([round(df_proc['water_temp_int'].min(),4), round(df_proc['water_temp_int'].max(),4)], 'Internal Temp', 'C')
        ...
        self.add_bounds_stat([round(df_proc['water_temp_ext'].min(),4), round(df_proc['water_temp_ext'].max(),4)], 'External Temp', 'C')
        ...
        visualizer_data_obj['data'] = json.loads(df_proc[['date_time','water_temp_int']].to_json(orient='values'))
        visualizer_data_obj['unit'] = 'C'
        visualizer_data_obj['label'] = 'Internal Water Temp'
        ...
        visualizer_data_obj['data'] = json.loads(df_proc[['date_time','water_temp_ext']].to_json(orient='values'))
        visualizer_data_obj['unit'] = 'C'
        visualizer_data_obj['label'] = 'External Water Temp'
        self.add_visualization_data(deepcopy(visualizer_data_obj))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Parse SBE45 TSG with SBE38 temp sensor data')
    ...
    ovdm_parser = TSGParser(start_dt=parsed_args.startDT, stop_dt=parsed_args.stopDT, time_format=parsed_args.timeFormat)
