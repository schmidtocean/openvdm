...
DESCRIPTION:  Parse the supplied fluorometer data and return the json-formatted string
...
# $FLUO,{Date:S},{Time:S},{ChlSig:g},{NU:d},{ChlSigRaw:d},{Therm:d}*{CheckSum:x}
RAW_COLS = ['date_time','data'] # OpenRVDAS style

# RAW_COLS = ['date_time','hdr','sensor_date','sensor_time','chlorophyll_signal','nu','raw_chlorophyll_signal','therm_checksum'] # OpenRVDAS style
# RAW_COLS = ['date','time','data'] # SCS style
PROC_COLS = ['date_time','nu']
...
class FluoroParser(OpenVDMCSVParser):
    ...
    def __init__(self, start_dt=None, stop_dt=None, time_format=None, skip_header=False, use_openvdm_api=False, chlorophyll_signal=False):
        ...
        self.chlorophyll_signal = chlorophyll_signal
        self.data_to_raw_col = ['sensor_date','sensor_time','nu','raw_chlorophyll_signal','therm']

        if chlorophyll_signal is True:
            self.data_to_raw_col.insert(2, 'chlorophyll_signal')
    ...
    def process_file(self, filepath): # pylint: disable=too-many-locals,too-many-branches,too-many-statements
        ...
        self.add_bounds_stat([int(df_proc['nu'].min()), int(df_proc['nu'].max())], 'Chlorophyll Count Bounds', 'counts')
        self.add_value_validity_stat([len(df_proc[(df_proc['nu'] < MIN_CHLOROPHYLL_COUNTS) | (df_proc['nu'] > MAX_CHLOROPHYLL_COUNTS)]),len(df_proc[(df_proc['nu'] >= MIN_CHLOROPHYLL_COUNTS) & (df_proc['nu'] <= MAX_CHLOROPHYLL_COUNTS)])], 'Chlorophyll Count Validity')
        ...
        error_rate = len(df_proc[(df_proc['nu'] < MIN_CHLOROPHYLL_COUNTS) | (df_proc['nu'] > MAX_CHLOROPHYLL_COUNTS)]) / len(df_proc)
        if error_rate > .25:
            self.add_quality_test_failed("Chlorophyll Counts")
        elif error_rate > .10:
            self.add_quality_test_warning("Chlorophyll Counts")
        else:
            self.add_quality_test_passed("Chlorophyll Counts")
        ...
        visualizer_data_obj['data'] = json.loads(df_proc[['date_time','nu']].to_json(orient='values'))
...
if __name__ == "__main__":
    ...
    ovdm_parser = FluoroParser(start_dt=parsed_args.startDT, stop_dt=parsed_args.stopDT, time_format=parsed_args.timeFormat, use_openvdm_api=False)
