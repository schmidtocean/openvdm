...
USAGE:  hdt_parser.py [-h] [-v+] [--timeFormat] [--startDT] [--stopDT] <dataFile>

DESCRIPTION:  Parse the supplied NMEA HDT data and return the json-formatted string
...
RAW_COLS = ['date_time','hdr','heading','checksum'] #OpenRVDAS style
# RAW_COLS = ['date','time','hdr','heading','checksum'] # SCS style
PROC_COLS = ['date_time','heading']
...
class HDTParser(OpenVDMCSVParser):
    """
...
if __name__ == "__main__":
    ...
    ovdm_parser = HDTParser(start_dt=parsed_args.startDT, stop_dt=parsed_args.stopDT, time_format=parsed_args.timeFormat)
