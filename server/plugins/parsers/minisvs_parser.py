...
USAGE:  minisvs_parser.py [-h] [-v+] [--timeFormat] [--startDT] [--stopDT] <dataFile>
...
class MiniSVSParser(OpenVDMCSVParser):
    ...
    def process_file(self, filepath): # pylint: disable=too-many-locals,too-many-branches,too-many-statements
            ...
            with open(filepath, mode='r', encoding="utf-8") as csvfile:
                ...
                for lineno, line in enumerate(reader):
                    ...
                    try:
                        ...
                        # line_temperature, line_sound_spd = line['data'].strip().split()
                        line_sound_spd = line['data'].strip()
                        sound_spd = float(line_sound_spd)
                        ...
                        raw_into_df['sound_spd'].append(sound_spd)
        ...
        self.add_bounds_stat([round(df_proc['sound_spd'].min(),2), round(df_proc['sound_spd'].max(),2)], 'Sound Speed Bounds', 'm/s')
        ...
        visualizer_data_obj['data'] = json.loads(df_proc[['date_time','sound_spd']].to_json(orient='values'))
        visualizer_data_obj['unit'] = 'm/s'
        visualizer_data_obj['label'] = 'Sound Speed'
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Parse MiniSVS sensor data')
