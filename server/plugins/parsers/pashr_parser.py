#!/usr/bin/env python3
"""
FILE:  pashr_parser.py

USAGE:  pashr_parser.py [-h] [-v+] [--timeFormat] [--startDT] [--stopDT] <dataFile>

DESCRIPTION:  Parse the supplied NMEA PASHR data and return the json-formatted
...
PROC_COLS = ['date_time','heading','pitch', 'roll', 'heave']
...
class PashrParser(OpenVDMCSVParser):
    """
    ...
    def process_file(self, filepath): # pylint: disable=too-many-locals,too-many-branches,too-many-statements
            ...
            with open(filepath, mode='r', encoding="utf-8") as csvfile:
                ...
                for lineno, line in enumerate(reader):
                    ...
                    try:
                        ...
                        raw_into_df['heading'].append(heading)
                        raw_into_df['pitch'].append(pitch)
                        raw_into_df['roll'].append(roll)
                        raw_into_df['heave'].append(heave)
        ...
        self.add_bounds_stat([round(df_proc['pitch'].min(),3), round(df_proc['pitch'].max(),3)], 'Pitch Bounds', 'deg')
        self.add_bounds_stat([round(df_proc['roll'].min(),3), round(df_proc['roll'].max(),3)], 'Roll Bounds', 'deg')
        ...
        visualizer_data_obj['data'] = json.loads(df_proc[['date_time','heading']].to_json(orient='values'))
        visualizer_data_obj['unit'] = 'deg'
        visualizer_data_obj['label'] = 'Heading'
        self.add_visualization_data(deepcopy(visualizer_data_obj))

        visualizer_data_obj['data'] = json.loads(df_proc[['date_time','pitch']].to_json(orient='values'))
        visualizer_data_obj['unit'] = 'deg'
        visualizer_data_obj['label'] = 'Pitch'
        self.add_visualization_data(deepcopy(visualizer_data_obj))

        visualizer_data_obj['data'] = json.loads(df_proc[['date_time','roll']].to_json(orient='values'))
        visualizer_data_obj['unit'] = 'deg'
        visualizer_data_obj['label'] = 'Roll'
        self.add_visualization_data(deepcopy(visualizer_data_obj))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Parse NEMA PASHR data')
    ...
    ovdm_parser = PashrParser(start_dt=parsed_args.startDT, stop_dt=parsed_args.stopDT, time_format=parsed_args.timeFormat)
