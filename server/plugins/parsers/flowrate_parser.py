#!/usr/bin/env python3
"""
FILE:  flowrate_parser.py

USAGE:  flowrate_parser.py [-h] [-v+] [--timeFormat] [--startDT] [--stopDT] <dataFile>

DESCRIPTION:  Parse the supplied flowrate file and return the json-formatted string
...
RAW_COLS = ['date_time','hdr','flowrate'] # OpenRVDAS style
# RAW_COLS = ['date','time','flowrate'] # SCS style
PROC_COLS = ['date_time','flowrate']
...
class FlowrateParser(OpenVDMCSVParser):
    """
    ...
    def process_file(self, filepath): # pylint: disable=too-many-locals,too-many-branches,too-many-statements
            ...
            with open(filepath, 'r') as csvfile:
                ...
                for lineno, line in enumerate(reader):
                    ...
                    try:
                        ...
                        raw_into_df['flowrate'].append(flowrate)
        ...
        self.add_bounds_stat([round(df_proc['flowrate'].min(),3), round(df_proc['flowrate'].max(),3)], 'Flowrate Bounds', 'l/min')
        ...
        visualizer_data_obj['data'] = json.loads(df_proc[['date_time','flowrate']].to_json(orient='values'))
        visualizer_data_obj['unit'] = 'l/min'
        visualizer_data_obj['label'] = 'Flowrate'
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Parse flowrate data')
    ...
    ovdm_parser = FlowrateParser(start_dt=parsed_args.startDT, stop_dt=parsed_args.stopDT, time_format=parsed_args.timeFormat, use_openvdm_api=False)
