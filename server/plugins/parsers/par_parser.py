...
USAGE:  par_parser.py [-h] [-v+] [--timeFormat] [--startDT] [--stopDT] <dataFile>
...
RAW_COLS = ['date_time','hdr','irradiance','voltage'] # OpenRVDAS style
# RAW_COLS = ['date','time','hdr','irradiance','voltage'] # SCS style
PROC_COLS = ['date_time','irradiance']
...
class PARParser(OpenVDMCSVParser):
    ...
    def process_file(self, filepath): # pylint: disable=too-many-locals,too-many-branches,too-many-statements
            ...
            with open(filepath, mode='r', encoding="utf-8") as csvfile:
                ...
                for lineno, line in enumerate(reader):
                    ...
                    try:
                        ...
                        irradiance = float(line['irradiance'])
                        ...
                        raw_into_df['irradiance'].append(irradiance)
        ...
        self.add_bounds_stat([round(df_proc['irradiance'].min(),3), round(df_proc['irradiance'].max(),3)], 'Irradiance', 'uE/cm2sec')
        ...
        visualizer_data_obj['data'] = json.loads(df_proc[['date_time','irradiance']].to_json(orient='values'))
        visualizer_data_obj['unit'] = '\u03BCE/cm2sec'
        visualizer_data_obj['label'] = 'Irradiance'
