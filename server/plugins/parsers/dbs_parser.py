...
USAGE:  dbs_parser.py [-h] [-v+] [--timeFormat] [--startDT] [--stopDT] <dataFile>

DESCRIPTION:  Parse the supplied NMEA DBS file and return the json-formatted string
...
RAW_COLS = ['date_time','hdr','depth_f','f','depth_m','m','depth_F','F_checksum'] # OpenRVDAS style
# RAW_COLS = ['date','time','hdr','depth_f','f','depth_m','m','depth_F','F_checksum'] # SCS style
PROC_COLS = ['date_time','depth_m']
...
class DBSParser(OpenVDMCSVParser):
    """
    ...
    def process_file(self, filepath): # pylint: disable=too-many-locals,too-many-branches,too-many-statements
            ...
            with open(filepath, mode='r', encoding="utf-8") as csvfile:
                ...
                for lineno, line in enumerate(reader):
                    ...
                    try:
                        ...
                        raw_into_df['depth_m'].append(depth)
        ...
        self.add_bounds_stat([round(df_proc['depth_m'].min(),3), round(df_proc['depth_m'].max(),3)], 'Depth Bounds', 'm')
        self.add_value_validity_stat([len(df_proc[(df_proc['depth_m'] >= MIN_DEPTH)]),len(df_proc[(df_proc['depth_m'] < MIN_DEPTH)])], 'Depth Validity')
        ...
        error_rate = len(df_proc[(df_proc['depth_m'] < MIN_DEPTH)]) / len(df_proc)
        if error_rate > .25:
        ...
        visualizer_data_obj['data'] = json.loads(df_proc[['date_time','depth_m']].to_json(orient='values'))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Parse NMEA DBS data')
    ...
    ovdm_parser = DBSParser(start_dt=parsed_args.startDT, stop_dt=parsed_args.stopDT, time_format=parsed_args.timeFormat)
